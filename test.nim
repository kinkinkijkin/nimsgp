...
import src/nimsgp/[datatypes,topdraw,toptransform,shaderimp/core]
...
proc fill_surface (surf: var Surface, buf: ColourOutBufferOBUF) =
    var nearest = vec2(buf.len / screen_w, buf[0].len / screen_h)
    var death = cast[ptr UncheckedArray[byte]](surf.pixels)
    for x in 0..screen_w-1:
        for y in 0..screen_h-1:
            var posit = (((y * screen_w) + x) * 3)
